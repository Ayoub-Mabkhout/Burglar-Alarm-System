...
enum states
{
    ...
    ARMED,
    ALARM
}; // ENUM type for the alarm states (FSM states)
enum inputs
{
    BUTTON,
    CLOUD,
    INTRUDER_IN,
    INTRUDER_OUT
}; // ENUM type for the types of input the alarm FSM can take
...
void stateFSM()
{                                 // State FSM as described in the FSM Drawing
    ...
    switch (state)
    { // the main variable being set (output of the FSM) is alarmEnabled
    case OFF:
...
{                                // a function that toggles LED0 on and off and BUZZER HIGH/LOW pitch tone
...
void btnISR()
{                                     // input work with interrupts instead of polling, ISR's have to be carefully designed
...
void rlsISR()
{ // RLS ISR launches or stops the absence and presence timers,
  // which will eventually update the input for the FSM
    if (!digitalRead(RLS))
    { // if no one is detected, disable the absence timer and state the presence timer
    ...
    { // no intruder detected => stop the presence timer and start the absence timer
...
void absTimerISR()
{                         // ISR for the absence timer, called when intruder absent for 20s
    input = INTRUDER_OUT; //  => input becomes INTRUDER_OUT + execute FSM
    stateFSMFlag = true;
}

void setIntruderIn()
{                        // ISR for the presence timer, called when intruder present for 3s
...
int cloudArmDisarm(String code)
{ // ISR for cloud function to ARM/DISARM the BAS
    if (code.equals(cloudKey))
    {                        // check the cloud key given as argument
...
void loop()
{ // nearly all the different sections of the code are asynchronous
    if (stateFSMFlag)
    {                         // not much going on in the main loop except for executing the FSM when prompted
